{ lib, python3Packages, ffmpeg, fetchFromGitHub }:

python3Packages.buildPythonApplication {
  pname = "ffmpegsplitpy";
  version = "0.1.0";
  
  src = ./.; # Lokasi file Python Anda
  
  # Jika ini adalah aplikasi yang Anda buat sendiri, gunakan format src seperti ini:
  # Atau gunakan format ini jika Anda ingin menghostingnya di GitHub:
  /*
  src = fetchFromGitHub {
    owner = "username";
    repo = "ffmpegsplitpy";
  ...
  */
  ...
  propagatedBuildInputs = with python3Packages; [
    # Tidak ada dependensi Python khusus selain pustaka standar
  ];
  ...
  # Membuat wrapper untuk menjamin ffmpeg tersedia di PATH
  makeWrapperArgs = [
    "--prefix PATH : ${lib.makeBinPath [ ffmpeg ]}"
  ];
  
  # Membuat file Python dasar
  postUnpack = ''
    cat > $sourceRoot/ffmpegsplitpy.py << 'EOF'
#!/usr/bin/env python
...
EOF
  ...
  # Setup.py dasar untuk menangani instalasi
  postPatch = ''
    cat > setup.py << EOF
...
setup(
    name="ffmpegsplitpy",
    version="0.1.0",
    description="Split video files using ffmpeg",
    py_modules=["ffmpegsplitpy"],
    entry_points={
        'console_scripts': [
            'ffmpegsplitpy=ffmpegsplitpy:main',
        ],
    },
)
EOF
  ...
  meta = with lib; {
    description = "Utilitas untuk memecah file video menggunakan ffmpeg";
    longDescription = ''
      Alat command-line untuk memecah file video menjadi segmen-segmen yang lebih kecil
      berdasarkan waktu, jumlah chunk, atau ukuran file. Menggunakan ffmpeg di balik layar.
    '';
    homepage = "https://github.com/yourusername/ffmpegsplitpy";
    license = licenses.mit;
    maintainers = with maintainers; [ /* nama maintainer */ ];
