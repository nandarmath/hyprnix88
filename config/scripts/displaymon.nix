...
pkgs.python3Packages.buildPythonApplication rec {
  ...
  src = pkgs.writeTextDir "hyprdisplay.py" ''
    #!/usr/bin/env python3
    ...
    LOCK_FILE = "/tmp/hyprdisplay.lock"
  ...
  buildInputs = with pkgs; [
    bash
    coreutils
    systemd  # Untuk systemctl
  ];

  # Buat executable script
  installPhase = ''
    mkdir -p $out/bin
    cp ${src}/hyprdisplay.py $out/bin/hyprdisplay
    chmod +x $out/bin/hyprdisplay
    
    # Wrapper untuk memastikan dependencies tersedia
    wrapProgram $out/bin/hyprdisplay \
      --prefix PATH : ${pkgs.lib.makeBinPath [ 
        pkgs.hyprland 
        pkgs.bash 
        pkgs.coreutils 
        pkgs.systemd
        pkgs.procps  # Untuk pkill
      ]}
  '';
  ...
  meta = with pkgs.lib; {
    ...
    homepage = "https://github.com/your-repo/hyprdisplay";
    license = licenses.mit;
    maintainers = [ "your-name" ];
    platforms = platforms.linux;
  };

  # Wrapper program untuk dependencies
